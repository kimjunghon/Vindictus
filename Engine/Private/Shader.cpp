...
HRESULT CShader::Begin(_uint iPassIndex)
{
	...
	if (FAILED(m_pEffect->GetTechniqueByIndex(0)->GetPassByIndex(iPassIndex)->Apply(0, m_pDeviceContext)))
		return E_FAIL;

	m_pDeviceContext->IASetInputLayout(m_InputLayouts[iPassIndex]);
...
{
	ID3DX11EffectVariable* pVarialbe = m_pEffect->GetVariableByName(pConstantName);
	if (nullptr == pVarialbe)
		return E_FAIL;

	ID3DX11EffectMatrixVariable* pMatrixVariable = pVarialbe->AsMatrix();
