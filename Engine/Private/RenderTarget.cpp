...
{
	return pShader->Bind_SPV(pConstantName, m_pSRV);
}

void CRenderTarget::Clear()
{
	m_pDeviceContext->ClearRenderTargetView(m_pRTV, reinterpret_cast<_float*>(&m_vClearColor));
}
...
HRESULT CRenderTarget::Render(CShader* pShader, CVIBuffer_Rect* pVIBuffer)
{
	...
	if (FAILED(pShader->Bind_SPV("g_Texture", m_pSRV)))
		return E_FAIL;

	pShader->Begin(ENUM_CLASS(SHADER_DEFFERRED::DEBUG));
...
CRenderTarget* CRenderTarget::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, _uint iSizeX, _uint iSizeY, DXGI_FORMAT ePixelFormat, const _float4& vClearColor)
{
	...
	{
		MSG_BOX(TEXT("Failed Created : CRenderTarget"));
